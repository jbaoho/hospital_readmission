...
from .config import MODELS_DIR, RANDOM_STATE, ensure_directories
...
def get_torch_device(requested: str | None = None) -> str:
    """Resolve a torch device, validating requested CUDA/MPS devices."""
    if requested:
        ...
        if requested == "mps":
            ...
            if not torch.backends.mps.is_built():
                raise RuntimeError("MPS was requested, but this PyTorch build was not built with MPS.")
            if not torch.backends.mps.is_available():
                raise RuntimeError(
                    "MPS was requested, but torch.backends.mps.is_available() is False. "
                    "The Mac GPU is present, but the installed PyTorch runtime cannot allocate MPS tensors."
                )
...
def train_tabnet(
    ...
    ensure_directories()
    X_train = combined_tabnet_matrix(prepared.X_cat_train, prepared.X_num_train)
    X_val = combined_tabnet_matrix(prepared.X_cat_val, prepared.X_num_val)
    ...
    sample_weights = compute_sample_weight(class_weight="balanced", y=prepared.y_train.astype(int))
    fit_kwargs = {}
...
def predict_tabnet(model, X_cat: np.ndarray, X_num: np.ndarray) -> np.ndarray:
    X = combined_tabnet_matrix(X_cat, X_num)
    return model.predict_proba(X)[:, 1]


def run_deep(sample_size: int | None = None, quick: bool = False, skip_tabnet: bool = False) -> list[dict]:
    ...
    mlp, _ = train_mlp(prepared, epochs=mlp_epochs, patience=2 if quick else 5)
    val_proba = predict_mlp(mlp, prepared.X_cat_val, prepared.X_num_val)
    threshold = choose_threshold_max_f1(prepared.y_val, val_proba)
    test_proba = predict_mlp(mlp, prepared.X_cat_test, prepared.X_num_test)
    rows.append(metrics_row("MLP", "test", prepared.y_test, test_proba, threshold, "val_max_f1"))

    if not skip_tabnet:
        try:
            tabnet = train_tabnet(prepared, quick=quick)
...
def main() -> None:
    ...
    rows = run_deep(sample_size=args.sample_size, quick=args.quick, skip_tabnet=args.skip_tabnet)
    for row in rows:
        print(row)
